...
describe LogStasher do
  ...
  describe '.appened_default_info_to_payload' do
    ...
    it 'appends default parameters to payload' do
      ...
      expect(LogStasher::CustomFields.custom_fields).to eq %i[ip route request_id parameters]
    end
    ...
    it 'does not include parameters when not configured to' do
      ...
      expect(LogStasher::CustomFields.custom_fields).to eq %i[ip route request_id]
    end
  end
  ...
  describe '.append_custom_params' do
    ...
    it 'defines a method in ActionController::Base' do
      expect(ActionController::Base).to receive(:send).with(:define_method, :logstasher_add_custom_fields_to_payload,
                                                            &block)
  ...
  describe '.add_custom_fields_to_request_context' do
    let(:block) { ->(_, _) {} }
    it 'defines a method in ActionController::Base' do
      expect(ActionController::Base).to receive(:send).with(:define_method,
                                                            :logstasher_add_custom_fields_to_request_context, &block)
      expect(ActionController::Metal).to receive(:send).with(:define_method,
                                                             :logstasher_add_custom_fields_to_request_context, &block)
  ...
  describe '.log' do
    ...
    it 'allows specification of tags' do
      ...
      LogStasher.log('warn', 'main message', { other: 'data', tags: %w[a b] })
    end
  ...
  describe '.watch' do
    ...
    describe 'store' do
      it 'stores the events in a store with the event\'s name' do
        probe = ->(*_args, store) { store[:foo] = :bar }
